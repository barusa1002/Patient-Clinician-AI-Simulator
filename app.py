...
# JSタイマーが /?expired=1 へリダイレクトした後、
# セッションが残っていれば確実にクリアする
if st.query_params.get("expired") == "1" and st.session_state.get("access_token"):
    logout()
    # logout() の中で st.rerun() が呼ばれるので以下には到達しない
...
(function() {{
    var p = window.parent;
    // rerun のたびにこのスクリプトが再実行される。
    // 前のタイマーをキャンセルして新しいタイマーをセットすることで
    // 「操作のたびにリセット・30分無操作で発火」を実現する。
    if (p.__idleTimer) {{
        clearTimeout(p.__idleTimer);
    }}
    p.__idleTimer = setTimeout(function() {{
        p.__idleTimer = null;
...
def init_chat_session(mode, selected):
    ...
    _no_thinking = (
        ...
        "「RESPONSE:」「PRESENTER:」「SPEAKER:」「回答:」「発言:」など、"
        "いかなるラベル・話者名・英単語＋コロンも先頭に付けることを禁止。セリフ本文から直接書き始めること。"
        "「課題は」「〜すべき」「〜が最も無難」「〜が適切」「〜を返す」"
        "「〜はず」「〜べき」など推論を示す表現で始まる文は絶対に出力禁止。"
    )

    # 聞かれていない項目まで先回りして答えてしまうと、学生がその項目を
    # 質問する練習にならず、評価でも「聞けた」ことにならない。
    # 各シナリオにも同趣旨の指示があるが守られないため、
    # 最優先ルールとして具体例つきで最後に付ける。
    _answer_only = (
        "\n\n【絶対ルール - 最優先】"
        "相手に聞かれたことだけに答えること。聞かれていないことは、"
        "たとえ設定に書かれていても自分からは一切話さない。"
        "特に、アレルギー歴・副作用歴・併用薬・市販薬・サプリメント・健康食品・"
        "既往歴・生活習慣（喫煙・飲酒・運転・仕事）・飲み忘れ・不安や心配ごとは、"
        "相手がその項目を直接たずねるまで一言も触れてはならない。"
        "「〜もありません」「〜も飲んでいません」のように、"
        "聞かれていない項目を「〜も」で付け足す言い方は禁止。"
        "相手が1回の発言で複数のことを聞いた場合は、その聞かれた範囲だけ答える。"
        "\n【良い例】"
        "「今まで薬で合わなかったものはありますか」→「いいえ、特にないです。」"
        "／「今飲んでいる薬はありますか」→「特にないですね。」"
        "\n【悪い例（絶対にしない）】"
        "「今まで薬で合わなかったものはありますか」→"
        "「いいえ、特にないです。アレルギーもありません。」"
        "／「今飲んでいる薬はありますか」→"
        "「特にないですね。サプリメントも飲んでないです。」"
    )
